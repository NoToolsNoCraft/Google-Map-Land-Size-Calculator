function initMap() {
    ...
    document.getElementById('area').addEventListener('click', function (event) {
        ...
        console.log("Clicked element:", target); // Log clicked element
        if (target.tagName.toLowerCase() === 'button' && target.hasAttribute('data-land-index')) {
            console.log("Button detected with attributes", target.dataset); // Log button attributes
            const landIndex = parseInt(target.getAttribute('data-land-index'), 10);
            const exclusionIndex = target.hasAttribute('data-exclusion-index') ? parseInt(target.getAttribute('data-exclusion-index'), 10) : null;
    
            if (exclusionIndex !== null) {
                console.log(`Removing exclusion at landIndex: ${landIndex}, exclusionIndex: ${exclusionIndex}`);
                removeExclusion(landIndex, exclusionIndex);
            } else {
                console.log(`Removing land at landIndex: ${landIndex}`);
